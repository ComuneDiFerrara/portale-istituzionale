...
class FeedUtility extends \yii\base\BaseObject
{
    ...
    public function addEntryInFeed($feed, $entity, $key)
    {
        ...
        } else if (method_exists($entity, 'getFullViewUrl')) {
            if ($this->module->enableUseFrontendView) {
                $entity->useFrontendView = true;
            } else if ($this->module->enableUsePrettyUrl) {
                $entity->usePrettyUrl = true;
            }
            $url = \Yii::$app->urlManager->createAbsoluteUrl($entity->getFullViewUrl());
        } else {
