...
<script>
        ...
        for ($i=0; $i<otherButtonToDeactive.length; $i++){
         ...
         if(filtroAttivo=='filtro-tutto'){
            ...
            for($i=0;$i<cardToDisplay.length;$i++){
                ...
                if(cardToDisplay[$i].getAttribute('data-filter') && cardToDisplay[$i].getAttribute('data-filter').includes(filtroAttivo)){
                    cardToDisplay[$i].style.display='block';
    ...
    <!--parte filtri-->
    <div id="filtri-ricerca-modulistica" class="d-flex flex-column flex-md-row justify-content-between pb-2 border-bottom">
        <p class="h3">Tutti i moduli</p>
        
        <div class="tag-filtri-container d-flex flex-wrap">
            <div class="ml-2 mb-2 d-inline">    
                <button id="filtro-tutto-modulistica" class="active active-filter tag-filter btn btn-icon btn-outline-primary btn-sm align-top" onclick="resetFilterTag('filtro-tutto')">
                    <span>Tutto</span>
                </button>
            </div>
            ...
            <?php foreach($tagFilter as $filter){ ?>
                <div class="ml-2 mb-2 d-inline">
                    <button id="filtro-<?=str_replace(' ', '_', strtolower($filter->nome)) ?>-modulistica" class=" tag-filter btn btn-icon btn-outline-primary btn-sm align-top" onclick="resetFilterTag('filtro-<?= $filter->nome?>'.toLowerCase())">   
            ...
            <div class="ml-2 mb-2 d-inline">
                <button  id="more-tag-modulistica" type="button"  class="btn btn-icon btn-outline-primary btn-sm align-top h-100" data-toggle="modal" data-target="#more-option-filter-modal" onclick="onlyOne()">
    ...
    <div id="modulo-backend-modulistica" class="modulo-backend-<?= $modelLabel ?> <?= $cssClass ?> pt-4">
                ...
                <div class="form-group mt-5" >
                    ...
                    <div class="advanced-subnav right-subnav">
                        ...
                        <div class="link-list-wrapper">
                            <div class="link-list collapse mb-0" id="altri-arg-modulistica">
                                <div >
                                   <?php } if($showTag) { ?>
                        ...
                        <p class="espandi-voci mt-4 ">
                                <a href="#altri-arg-modulistica" id="menu-lista-button" role="button" aria-expanded="false" aria-controls="altri-arg-modulistica" data-toggle="collapse" class="collapsed link-altre-voci d-flex align-items-center text-decoration-none">
