    ...
    public function frontend(array $params = array()) 
    {
        ...
        if (!is_null($nav)) 
        {
            ...
            if (!is_null($page)) 
            {
                ...
                {
                    $prev=\luya\cms\models\NavItemPageBlockItem::findOne(['id' => $model->prev_id]);
                    if((!empty($prev)&&!$prev->is_hidden))
                    {
                        ...
                        if($values->title_element == $this->getVarValue('header', ''))
                        {
                            ...
                            $linkNoSpace = str_replace(' ', '_', $link);
                            $htags = \yii\helpers\Html::a($values->content, \yii\helpers\Url::to($linkNoSpace));  //$this->getTextBetweenTags($values->content,$configs['header']);
                            if(!empty($htags))
                            {
                                $titles[] = $htags;
                            }
