...
class CmsapiController extends Controller {
     ...
     * @param string $term
     */
    public function actionAutoComplete($term, $baseUrl) 
    {
        $iconClass = 'icon icon-sm icon-primary mr-2';
        $response = "";
        $request = Yii::$app->request;
        if ($request->isAjax) {
            if ($request->isGet) {
                ...
                $query->limit(9);
                $query->boolQuery()->addMustLike("title", $term);
                $query->boolQuery()->addFilter('public', "1");
                $results = $query->all(false);
                foreach ($results['hits']['hits'] as $result) {
                    $icon = $this->calcIcon($result['_type']);
                    $response .= '<a class="dropdown-item px-2 text-decoration-none d-flex align-items-center" role="option" href="#"><svg class="'.$iconClass.'">
                           <use xlink:href="'.$baseUrl.'/sprite/material-sprite.svg#'.$icon.'"></use>
                          </svg>'.$result['_source']['title'] .'</a>';
                }
            }
        }
        return $response;
    ...
    private function calcIcon($className)
    {
        $icon = "";
        switch (trim($className)) {
            case "open20_agid_organizationalunit_models_AgidOrganizationalUnit":
                $icon='bank-outline';
                break;
            case "open20_amos_documenti_models_Documenti":
                $icon='file-document-outline';
                break;
            case "open20_agid_service_models_AgidService":
                $icon='cog';
                break;
            case "open20_amos_events_AmosEvents":
                $icon='calendar';
                break;   
            case "open20_agid_person_models_AgidPerson":
                $icon='card-account-details-outline';
                break;  
            case "open20_amos_news_models_News":
                $icon='newspaper';
                break;
