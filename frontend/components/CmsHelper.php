...
class CmsHelper
{
    ...
    public static function MenuRender($menu)
    {
        ...
        foreach ($menu as $item) {
            $html .= '<li class="' . ($item->isActive ? ' active preopened' : '') . ($item->hasChildren
                ? ($item->depth == 1 ? ' dropdown' : ' dropdown-submenu') : '') . '">';
            $html .= '<a class="' . ($item->hasChildren ? ' nav-link dropdown-toggle'
                : '') . '" ' . ($item->hasChildren ? ' data-toggle="dropdown"' : '') . ' href="' . ($item->hasChildren
                ? "javascript:void(0)" : $item->link) . '">' . $item->title;
            $html .= $item->hasChildren ? '<span class="am am-chevron-down"></span>'
                : '';
            $html .= '</a>';
            if ($item->hasChildren) {
                $html .= '<ul class="dropdown-menu dropdown-menu-left">';
                $html .= static::MenuRender(Yii::$app->menu->getLevelContainer($item->depth
                    + 1, $item));
                $html .= '</ul>';
            }
            $html .= '</li>';
     ...
     * @param array $menu
     */
    public static function BiHamburgerMenuRender(
        $menu,
        $iconSubmenu,
            ...
            if ($item->hasChildren && !($onlyFirstLevel)) {
                $listMenu .= Html::tag(
                    ...
                    Html::a(
                        ...
                        null,
                        [
                            'class' => 'nav-link dropdown-toggle' . ' ' . ($item->isActive
                                ? 'active' : ''),
                            'target' => '_self',
                            'title' => $item->title,
                            'href' => '#',
