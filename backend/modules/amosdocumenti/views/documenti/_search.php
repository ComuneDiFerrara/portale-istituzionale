...
<div class="documenti-search element-to-toggle" data-toggle-element="form-search">
    ...
    <div class="col-sm-6 col-lg-4">
        <?php 
            $status = $model->getWorkflowSource()->getWorkflow($model::DOCUMENTI_WORKFLOW)->getAllStatuses();
            foreach ($status as $key => $value) {
                if( ($key == Documenti::DOCUMENTI_WORKFLOW_STATUS_BOZZA) || ($key == Documenti::DOCUMENTI_WORKFLOW_STATUS_VALIDATO) ){
                    $array_status[$key] = AmosDocumenti::t('amosdocumenti', $value->getLabel());
                }
            }
        ?>
        <?= 
